...
const selectMasterData = async (data) => {
  ...
  let query = `SELECT kyc.*, distributor_details.*, ase_details.*
  FROM Retailerkyc_details as kyc 
  INNER JOIN distributor_details ON distributor_details.distributorcode = kyc.aw_code
  INNER JOIN ase_details ON kyc.ase_email = ase_details.ase_email_id
  WHERE 1=1`;

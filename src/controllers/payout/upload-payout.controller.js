...
const uploadPayoutFile = async (req, res) => {
  ...
  try {
    ...
    const promiseArray = data.map(async (element) => {
      const SalaryPayout = parseFloat(element[6]);
      const IncentivePayout = parseFloat(element[7]);

      let typeText;
      if (!isNaN(SalaryPayout) && isNaN(IncentivePayout)) {
        typeText = 'Salary';
      } else if (isNaN(SalaryPayout) && !isNaN(IncentivePayout)) {
        typeText = 'Incentive';
      } else if (!isNaN(SalaryPayout) && !isNaN(IncentivePayout)) {
        typeText = 'Salary + Incentive';
      } else {
        typeText = '';
      }
      ...
      let excelObject = {
        name: element[0],
        aseemailid: element[1],
        dsrcode: element[2],
        dsrname: element[3],
        distributor_name: element[4],
        distributor_code: element[5],
        SalaryPayout: SalaryPayout || '',
        IncentivePayout: IncentivePayout || '',
        payout_month: element[8],
        Type: typeText,
        Amount: (SalaryPayout || 0) + (IncentivePayout || 0),
      };
    ...
    Promise.all(promiseArray)
      .then(async () => {
        const fields = [
          ...
          { label: "SO EMAIL", value: 'aseemailid' },

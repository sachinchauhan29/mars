...
const editKycDataView = async (req, res, next) => {
  let allDetails = await filterEditKycData(req.query);
  let awsmCity = await getAWSMCity()
  res.render('edit-kyc-data', { user: res.userDetail, kycData: allDetails, QueryData: req.query, awsmCity, notification: res.notification });
}
